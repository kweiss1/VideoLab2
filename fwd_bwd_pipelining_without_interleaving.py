...
def forward_backward_pipelining_without_interleaving(
    ...
    **kwargs,
    ...
    # Ensure the model is a list
    model = listify_model(model)
    if len(model) != 1:
        raise RuntimeError(f"`model` must be a single `nn.Module`, got: {type(model)}")
    model = model[0]

    # Sync context handlers for gradient reductions
    sync_context_handler = custom_sync_context_handler or contextlib.nullcontext
    sync_context = None
    ...
    # Storage for input/output tensors
    input_tensors = []
    output_tensors = []
    losses_reduced = []
    ...
    # Before running 1F1B, receive the first forward tensor if needed
